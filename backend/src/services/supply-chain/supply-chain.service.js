...
import { toBatchDetail } from "./supply-chain.mappers.js";
...
export class SupplyChainService {
    ...
    async enrichBatchWithOrgDetails(batch) {
        ...
        let [manufacturer, owner, drugCategory, index, createEvent] = await Promise.all([
            ...
            import("../../models/batch/batch-geo-event.model.js").then(m =>
                m.BatchGeoEvent.findOne({ batchID: targetBatchId, eventType: "CREATE" }).lean()
            )
        ]);
        ...
        if (index && (index.transferStatus !== ledgerTransferStatus || index.ownerMSP !== ledgerOwner || (ledgerOwnerId && index.ownerId !== ledgerOwnerId))) {
        ...
        const enriched = {
            ...
            metadata: {
                ...
                ...(batch.metadata || {}),
                certificates: batch.metadata?.certificates || index?.qrMetadata?.certificates || createEvent?.metadata?.certificates || drugCategory?.certificates || [],
            },
        };
    ...
    async verifyProduct(imageBuffer, traceId = "", options = {}) {
        ...
        const enrichedBatch = await this.enrichBatchWithOrgDetails(result.batch);
        ...
        return {
            ...
            protectedQrRecord,
            aiVerification,
            batchInfo: toBatchDetail(enrichedBatch),
            safetyStatus: result.safetyStatus,
        };
    }
    ...
    async shipBatch(batchID, transferTarget, actor) {
        const receiverMSP = String(transferTarget?.receiverMSP || "");
        const targetDistributorUnitId = normalizeDistributorUnitId(
            transferTarget?.targetDistributorUnitId,
        );
        const actorDistributorUnitId = normalizeDistributorUnitId(
        ...
        if (isDistributorToDistributorTransfer && !targetDistributorUnitId) {
            throw new HttpException(
                ...
                "targetDistributorUnitId is required for inter-distributor transfers",
            );
        }

        if (
            isDistributorToDistributorTransfer &&
            !actorDistributorUnitId
        ) {
            throw new HttpException(
                403,
        ...
        try {
            ...
            const updatePayload = {
                ...
                targetOwnerId: transferTarget?.targetOwnerId || "",
                updatedAt: new Date()
            };
    ...
    async confirmDeliveredToConsumption(batchID, actor) {
        return this.ledgerRepository.confirmDeliveredToConsumption(actor, batchID);
    }
    ...
    async emergencyRecall(batchID, actor, note = "") {
        ...
        try {
            ...
            const lastEvent = await import("../../models/batch/batch-geo-event.model.js").then(m => 
                m.BatchGeoEvent.findOne({ batchID }).sort({ occurredAt: -1 }).lean()
            );
            ...
            await createBatchGeoEvent(batchID, {
                eventType: "RECALL",
                source: "SYSTEM",
                lat: lastEvent?.lat || 10.762622, // Default to HCM if unknown
                lng: lastEvent?.lng || 106.660172,
                address: lastEvent?.address || "Hệ thống ghi nhận thu hồi",
                note: note || `Lệnh thu hồi khẩn cấp được phê duyệt bởi ${actor.username}`,
                metadata: {
                    actorId: actor.id,
                    recallNote: note || ""
                }
