...
test("integration: distributor ship requires target distributor unit", async (t) => {
    const server = await createServer();
    t.after(() => server.close());

    const token = signToken("Distributor", "DistributorMSP", "dist-unit-a");

    const response = await fetch(
        `${server.baseUrl}/api/v1/batches/BATCH_001/ship`,
        {
            method: "POST",
            headers: {
    ...
    assert.equal(body.error.code, "TARGET_DISTRIBUTOR_UNIT_REQUIRED");
});

test("integration: distributor ship rejects same target distributor unit", async (t) => {
    const server = await createServer();
    t.after(() => server.close());
    ...
    const body = await response.json();
    assert.equal(body.success, false);
    assert.equal(body.error.code, "SAME_DISTRIBUTOR_UNIT_TRANSFER_NOT_ALLOWED");
});

test("integration: distributor cross-unit ship passes API validation", async (t) => {
    const server = await createServer();
    t.after(() => server.close());
