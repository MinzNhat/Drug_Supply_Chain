...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config ${TEST_NETWORK_HOME}/channel-artifacts/config_block.pb -o "$ORDERER_ENDPOINT" --ordererTLSHostnameOverride orderer1.drugguard.vn -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
