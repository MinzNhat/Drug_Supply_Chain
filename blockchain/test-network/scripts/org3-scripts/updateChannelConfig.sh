...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"DistributorMSP":.[1]}}}}}' ${TEST_NETWORK_HOME}/channel-artifacts/config.json ${TEST_NETWORK_HOME}/organizations/peerOrganizations/distributor.drugguard.vn/org3.json > ${TEST_NETWORK_HOME}/channel-artifacts/modified_config.json
{ set +x; } 2>/dev/null
...
setGlobals 2
set -x
peer channel update -f ${TEST_NETWORK_HOME}/channel-artifacts/org3_update_in_envelope.pb -c ${CHANNEL_NAME} -o localhost:7050 --ordererTLSHostnameOverride orderer1.drugguard.vn --tls --cafile "$ORDERER_CA"
{ set +x; } 2>/dev/null
