...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.regulator.drugguard.vn"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.manufacturer.drugguard.vn"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.distributor.drugguard.vn"
    PORT=11051
  else
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer1.drugguard.vn -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
