...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.drugguard.vn/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.drugguard.vn/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.drugguard.vn/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.drugguard.vn/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.drugguard.vn/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.drugguard.vn

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.drugguard.vn-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.drugguard.vn-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer1.drugguard.vn

    mkdir -p organizations/ordererOrganizations/drugguard.vn/ca
    mkdir -p organizations/ordererOrganizations/drugguard.vn/msp/cacerts
    mkdir -p organizations/ordererOrganizations/drugguard.vn/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/drugguard.vn/orderers
    mkdir -p organizations/ordererOrganizations/drugguard.vn/tlsca

    CERT_DIR=organizations/ordererOrganizations/drugguard.vn

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.drugguard.vn-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.drugguard.vn-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/drugguard.vn/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/drugguard.vn/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.drugguard.vn-cert.pem"
