...
run_full_suite() {
    ...
    run_step 4 "transfer-batch" "Validate ownership transfer workflow with dedicated ship/receive E2E flow." env STACK_BUILD_IMAGES=false STACK_BUILD_E2E_RUNNER=false "${RUNNER}" test-transfer
    run_step 5 "teardown" "Stop all services and clean runtime artifacts after validation." "${RUNNER}" down
}
...
run_single_mode() {
    case "${MODE}" in
        ...
        test)
            ...
            ;;
        transfer)
            run_step 4 "transfer-batch" "Validate ownership transfer workflow with dedicated ship/receive E2E flow." "${RUNNER}" test-transfer
            ;;
        down)
            run_step 5 "teardown" "Stop all services and clean runtime artifacts after validation." "${RUNNER}" down
            ;;
        ...
        *)
            ...
            echo "Usage: ./scripts/test-all.sh [full|prereq|up|test|transfer|down]"
            exit 1
            ;;
    esac
}
