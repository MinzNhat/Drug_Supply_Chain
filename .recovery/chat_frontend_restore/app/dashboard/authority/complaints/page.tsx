...
export default function ComplaintsPage() {
    ...
    const [selectedRow, setSelectedRow] = useState<SurveillanceItem | null>(null);
    ...
    // Auto-search by id if ?search=... is present in URL
    useEffect(() => {
            ...
            <div className="shrink-0 flex h-8 border-b border-border bg-card">
                <div className="flex-1 relative group">
                    ...
                    <input
                        ...
                        className="w-full h-full bg-transparent min-w-44 pl-8 pr-3 text-[11px] focus:outline-none placeholder:text-muted-foreground rounded-none"
                        value={searchInput}
        ...
        return () => {
                    ...
                    <table className="w-full border-collapse">
                        ...
                        <tbody>
                            {items.map((item) => (
                                <tr
                                    key={item.id}
                                    className="hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors border-b border-border last:border-b-0 cursor-pointer"
                                >
                                    <td className="px-3 border-r border-border">
                                        <div className="flex items-center justify-center gap-2">
                                            ...
                                            {item.status === "PENDING" ? (
                                                ...
                                                <div className="w-8 flex justify-center opacity-20">
                                                    <Check
                                                        size={10}
            ...
            <div className="shrink-0 h-8 bg-card border-t-0.5 border-border flex items-center px-3 justify-between">
                <div className="text-[9px] font-bold text-muted-foreground uppercase">
                    {t("trace_footer_total")}: {items.length}
                </div>
                ...
                customContent={
                    selectedRow && (
                        <div className="space-y-6 py-2">
                            {/* Status Management (only show historical note if processed) */}
